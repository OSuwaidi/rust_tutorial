...
fn main() -> Result<(), Error> {
    let mut n = String::new(); // Mutable variable "n" owns the data "String::new()" and its allocated memory
    stdin().read_line(&mut n)?; // Pass in a *mutable reference (pointer)* to "n" for the method to ***borrow***
    ...
    println!("text2: {text2}");
    let concat_text = text1 + " world"; // "concat_text" now *owns* "text1", hence; we can't call "text1" afterwards, unless we use "text1.clone()"
    println!("{concat_text}");
...
/*
In rust, every bit of allocated memory has exactly ONE owner only, and as soon as the owner gets deleted
...
Moreover, any variable that used to point (reference) to that moved variable will no longer be accessible (dangling pointer)
*/
...
(it's a relatively cheap operation to copy these primitive datatypes)
*/
...
Ans:
    ...
    When dealing with large or complex datatypes (hashmaps), it's more efficient to use a *reference* "&"; which essentially is a
