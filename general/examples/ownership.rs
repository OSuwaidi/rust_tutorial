...
/* Note: primitive datatypes: {integers, floats, booleans, tuples, and characters} all implement the "Copy" trait;
meaning they can be passed as arguments directly without using "&" and their value data will be copied */

/*
*** BIG QUESTION ***
Q.) When should I *move* ownership of a variable, "copy" a variable, or pass a *reference* to a variable?

Ans. When dealing with simple datatypes (e.g., small "structs" or "enums"), moving their ownership by passing them directly
    is more efficient, because transferring their ownership is cheap and doesn't involve any copying of the data, especially
    if the function/method needs to mutate the value.

    When dealing with primitive datatypes (that implement the "Copy" trait), copying the data is faster,
    and there is no need to manage their ownership or borrowing by passing them as references using the "&".

    When dealing with large or complex datatypes, it's more efficient to use a *reference* "&"; which essentially is a
    pointer to the original value, because it avoids the cost of copying the data.
 */
